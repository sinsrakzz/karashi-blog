# trace — HTML / CSS / JavaScript portfolio

Versi murni HTML, CSS, dan JavaScript (tanpa TypeScript, tanpa build step,
tanpa framework). Tinggal buka `index.html` di browser — selesai.
...
- `writeups/itechnocup2026.html` — halaman writeup lengkap ITechnoCup 2026 CTF
  Penyisihan (Forensic, Cryptography, Reverse Engineering, PWN, OSINT),
  ditulis manual jadi HTML statis (bukan lagi link ke PDF). Card writeup di
  homepage sudah diarahkan ke file ini lewat `href: "writeups/itechnocup2026.html"`
  di `script.js`.
...
3. Ganti `href` tiap writeup ke link post asli begitu sudah online.
